...
maybeUserIdentityToComponents (Just (UAuthIdentity uaid)) =
  -- TODO: maybe the outer email is "validated", and the one in uauth is not?  so they don't have to match?  but then UAuthIdentity should probably carry both?
  _
    (ewsEmail <$> uaid.uaEmail, Nothing, pure uaid, Nothing, Nothing)
