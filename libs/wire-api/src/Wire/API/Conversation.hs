...
accessRolesSchemaTuple =
  (,)
    <$> fst .= optFieldWithDocModifier "access_role" (DS.description ?~ "Deprecated, please use access_role_v2") (maybeWithDefault A.Null schema)
  ...
  ConversationMetadata
    ...
    <*> cnvmCreator
      .= optFieldWithDocModifier
        "creator"
        (DS.description ?~ "The creator's user ID")
    ...
    <*> cnvmMessageTimer
      .= optFieldWithDocModifier
        "message_timer"
        (DS.description ?~ "Per-conversation message timer (can be null)")
        (maybeWithDefault A.Null schema)
    ...
    $ ConversationList
      ...
      <*> convHasMore
        .= fieldWithDocModifier
          "has_more"
          (DS.description ?~ "Indicator that the server has more conversations than returned")
          schema
...
instance ToSchema ReceiptMode where
  schema =
    (S.schema . DS.description ?~ "Conversation receipt mode") $
    ...
    $ NewConv
      ...
      <*> newConvTeam
        .= maybe_
          ( optFieldWithDocModifier
              "team"
              (DS.description ?~ "Team information of this conversation")
              schema
          )
      <*> newConvMessageTimer
        .= maybe_
          ( optFieldWithDocModifier
              "message_timer"
              (DS.description ?~ "Per-conversation message timer")
              schema
          )
      <*> newConvReceiptMode .= maybe_ (optField "receipt_mode" schema)
      <*> newConvUsersRole
        .= ( fieldWithDocModifier "conversation_role" (DS.description ?~ usersRoleDesc) schema
      ...
      <*> newConvChannelAddPermission
        .= maybe_
          (optFieldWithDocModifier "add_permission" (DS.description ?~ "Channel add permission") schema)
      <*> newConvSkipCreator
        .= ( fromMaybe False
               <$> optFieldWithDocModifier
                 "skip_creator"
                 (DS.description ?~ "Don't add creator to the conversation, only works for team admins not wanting to be part of the channels they create.")
                 schema
           )
      <*> newConvParent
        .= maybe_
          ( optFieldWithDocModifier
              "parent"
              (DS.description ?~ "Parent conversation")
              schema
          )
...
toLegacyConversationMetadata meta =
  meta {cnvmGroupConvType = meta.cnvmGroupConvType >>= toGroupConvTypeLegacy}

toLegacyOwnConversation :: OwnConversation GroupConvType -> OwnConversation GroupConvTypeLegacy
toLegacyOwnConversation conv =
  conv {cnvMetadata = toLegacyConversationMetadata conv.cnvMetadata}

toLegacyConversation :: Conversation GroupConvType -> Conversation GroupConvTypeLegacy
toLegacyConversation conv =
  conv {metadata = toLegacyConversationMetadata conv.metadata}
...
toLegacyMLSOne2OneConversation conv =
  MLSOne2OneConversation
    { conversation = toLegacyOwnConversation conv.conversation,
      publicKeys = conv.publicKeys
    }
...
toLegacyCreateGroupConversation cgc =
  CreateGroupConversation (toLegacyConversation cgc.conversation) cgc.failedToAdd
