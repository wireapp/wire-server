...
testEmailVisibleToSelf =
  testProperty "should not contain email when email visibility is EmailVisibleToSelf" $
    \user lhStatus ->
      let profile = connectedProfile EmailVisibleToSelf user lhStatus
...
testEmailVisibleIfOnTeam =
  testProperty "should contain email only if the user has one and is part of a team when email visibility is EmailVisibleIfOnTeam" $
    \user lhStatus ->
      let profile = connectedProfile EmailVisibleIfOnTeam user lhStatus
       in (profileEmail profile === (userTeam user *> userEmail user))
            .&&. profileLegalholdStatus profile === lhStatus

testEmailVisibleIfOnSameTeam :: TestTree
testEmailVisibleIfOnSameTeam =
  testGroup "when email visibility is EmailVisibleIfOnSameTeam" [testNoViewerTeam, testViewerDifferentTeam, testViewerSameTeam]
  where
    testNoViewerTeam = testProperty "should not contain email when viewer is not part of a team" $
      \user lhStatus ->
        let profile = connectedProfile (EmailVisibleIfOnSameTeam Nothing) user lhStatus
         in (profileEmail profile === Nothing)
              .&&. profileLegalholdStatus profile === lhStatus
    ...
    testViewerDifferentTeam = testProperty "should not contain email when viewer is not part of the same team" $
      \viewerTeamId viewerMembership user lhStatus ->
        let profile = connectedProfile (EmailVisibleIfOnSameTeam (Just (viewerTeamId, viewerMembership))) user lhStatus
         in Just viewerTeamId /= userTeam user ==>
              ( profileEmail profile === Nothing
                  .&&. profileLegalholdStatus profile === lhStatus
              )

    testViewerSameTeam = testProperty "should contain email when viewer is part of the same team and is not an external member" $
      \viewerTeamId viewerMembership userNoTeam lhStatus ->
        let user = userNoTeam {userTeam = Just viewerTeamId}
            profile = connectedProfile (EmailVisibleIfOnSameTeam (Just (viewerTeamId, viewerMembership))) user lhStatus
         in Just viewerTeamId /= userTeam user ==>
...
testPublicProfile :: TestTree
testPublicProfile =
  testGroup
    "publicProfile"
    [ testProperty "never contains the email" $ \user lhStatus ->
        profileEmail (publicProfile user lhStatus) === Nothing
    ]
