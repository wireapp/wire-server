...
counterAdd :: MonadIO m => Word -> Counter -> m ()
counterAdd x (Counter c) = liftIO $ Atomic.incrCounter_ (fromIntegral x) c

-- counterIncr :: MonadIO m => Path -> Metrics -> m ()
-- counterIncr = counterAdd 1

counterIncr :: MonadIO m => Counter -> m ()
counterIncr = counterAdd 1
