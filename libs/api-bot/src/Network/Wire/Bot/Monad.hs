...
import Bilge (Manager, MonadHttp (..), withResponse)
...
mkBot tag user pw = do
  ...
  bot <-
    liftIO $
      Bot tag (botNetSettings env) user
        <$> newIORef (auth, aref)
...
consume bot e n = do
  ...
  forM_ (notifEvents n) $ \evt ->
    Logger.log l Info $
      botLogFields (botId bot) (botTag bot)
