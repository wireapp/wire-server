...
cqlUpsert = "INSERT INTO domain_registration (domain, domain_redirect, team_invite, idp_id, backend_url, team, dns_verification_token, auth_token_hash, authorized_team) VALUES (?,?,?,?,?,?,?,?,?)"

cqlSelect :: PrepQuery R (Identity Domain) (TupleType StoredDomainRegistration)
cqlSelect = "SELECT domain, domain_redirect, team_invite, idp_id, backend_url, team, dns_verification_token, auth_token_hash, authorized_team FROM domain_registration WHERE domain = ?"
