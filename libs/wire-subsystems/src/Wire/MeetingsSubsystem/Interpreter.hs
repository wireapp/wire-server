...
createMeetingImpl zUser newMeeting = do
  ...
  pushMeetingEvent zUser Nothing storedConv.localMembers MeetingEvent.MeetingCreate qMeetingId
...
updateMeetingImpl zUser meetingId update validityPeriod = do
  ...
  runMaybeT $ do
    ...
    lift $ pushMeetingEvent zUser Nothing conv.localMembers MeetingEvent.MeetingUpdate meetingId
...
deleteMeetingImpl zUser connId meetingId validityPeriod = do
  ...
  result <-
    runMaybeT $ do
      ...
      lift $ pushMeetingEvent zUser (Just connId) conv.localMembers MeetingEvent.MeetingDelete meetingId
  pure $ isJust result
...
pushMeetingEvent ::
  ...
  Qualified MeetingId ->
  Sem r ()
pushMeetingEvent lUser conn members ty qMeetingId = do
  now <- Now.get
  let evt = MeetingEvent.newEvent now ty qMeetingId
  pushNotifications
    [ def
