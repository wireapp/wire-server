...
newtype GroupInfoCheckEnabled
  = GroupInfoCheckEnabled {unGroupInfoCheckEnabled :: Bool}
...
isGroupInfoCheckEnabled ::
  (Member FeaturesConfigSubsystem r, Member (Input (Maybe GroupInfoCheckEnabled)) r) =>
  Maybe TeamId ->
  Sem r Bool
isGroupInfoCheckEnabled Nothing = pure False
isGroupInfoCheckEnabled (Just tid) = fmap isJust . runNonDetMaybe $ do
  global <- inputs $ fmap unGroupInfoCheckEnabled
