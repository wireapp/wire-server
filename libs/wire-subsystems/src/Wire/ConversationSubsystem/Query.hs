...
getUnqualifiedOwnConversation lusr cnv =
  getConversationAsMember (tUntagged lusr) (qualifyAs lusr cnv)
...
getConversation lusr cnv =
  foldQualified
    ...
    (error "TODO: gdf" . getRemoteConversation @r lusr)
    cnv
...
listConversations luser (Public.ListConversations ids) = do
  ...
  localInternalConversations <-
    E.getConversations foundLocalIds
      >>= filterM (\c -> pure $ isMember (tUnqualified luser) c.localMembers)
  localConversations <- (error "TODO: gdf") localInternalConversations
...
getMLSSelfConversation lusr = do
  ...
  maybe (createMLSSelfConversation lusr) pure mconv
