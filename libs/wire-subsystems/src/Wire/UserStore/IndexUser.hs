...
indexUserToDoc :: SearchVisibilityInbound -> Maybe Role -> Maybe [TeamId] -> IndexUser -> UserDoc
indexUserToDoc searchVisInbound mRole mCollaboratingTeams IndexUser {..} =
  if shouldIndex
    then
      UserDoc
        { udId = userId,
          ...
          udName = Just name,
          udTeam = teamId,
          udCollaboratingTeams = mCollaboratingTeams
        }
