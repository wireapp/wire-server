module Wire.HashPassword.InterpreterSpec (spec) where
  ...
  prop "verifyPasswordWithStatus: argon2id password" $ \(pw :: PlainTextPassword8) ->
    let argon2Hashed = runDependentEffects . runHashPassword fastArgon2IdOptions $ hashPassword8 rateLimitKey pw
     ...
     in statusWithScrypt === PasswordStatusOk
          ...
          .&.
          -- Maybe in future we want to consider changing this behaviour, it'll allow slowly changing
          -- argon2id options for passwords if needed.
          statusWithDifferentArgon2id === PasswordStatusOk

  it "should be able to passwords generated by old code" $ do
    forConcurrently_ goldenScryptPasswords $ \(plain, encodedHash) -> do
      case parsePassword encodedHash of
        Left err ->
          expectationFailure $
            "Failed to parse password: " <> Text.unpack encodedHash <> ", with error: " <> err
        Right hashed -> do
          let isVerified =
...
goldenScryptPasswords :: [(Text, Text)]
goldenScryptPasswords =
...
fastArgon2IdOptions :: PasswordHashingOptions
fastArgon2IdOptions =
  let hashParallelism = 4
   in PasswordHashingArgon2id $
        Argon2idOptions
          { iterations = 1,
...
slowArgon2IdOptions :: PasswordHashingOptions
slowArgon2IdOptions =
  PasswordHashingArgon2id $
    Argon2idOptions
      { iterations = 1,
        parallelism = 32,
        memory = 180224
