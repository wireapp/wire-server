...
-- | If this doens't terminate, it's because timeouts apply to every test case generated by
-- quickcheck, not to the property run itself.  Use `modifyMaxShrinks (const 0)` to work
-- around that.
    ...
    focus . prop "getGroups: pagination (happy flow)" $ do
      \(WithMods team1 :: WithMods '[AtLeastOneNonAdmin] ArbitraryTeam) numGroups pageSize ->
        expectRight . runDependencies (allUsers team1) (galleyTeam team1) . interpretUserGroupSubsystem $ do
          ...
          trace (show (numGroups, pageSize, groups, results)) . pure $
