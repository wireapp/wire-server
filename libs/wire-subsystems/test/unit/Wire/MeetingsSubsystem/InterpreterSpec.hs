...
-- | Decode all 'Push' payloads that are valid 'MeetingEvent.Event' values.
    ...
    it "emits a meeting.create event on successful create" $ do
      ...
      case result of
        ...
        Right (meeting, pushes) -> do
          ...
          let evt = head events
          evt.evtType `shouldBe` MeetingEvent.MeetingCreate
          evt.evtQualifiedId `shouldBe` meeting.meeting.id
    ...
    it "emits a meeting.update event on successful update" $ do
      ...
      case result of
        ...
        Right pushes -> do
          let events = extractMeetingEvents pushes
          length events `shouldBe` 1
          (head events).evtType `shouldBe` MeetingEvent.MeetingUpdate
    ...
    it "emits a meeting.delete event on successful delete" $ do
      ...
      case result of
        ...
        Right pushes -> do
          let events = extractMeetingEvents pushes
          length events `shouldBe` 1
          (head events).evtType `shouldBe` MeetingEvent.MeetingDelete
