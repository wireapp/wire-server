...
Wire Federation, once implemented, aims to allow multiple Wire-server :ref:`backends <backend>` to federate with each other. That means that a user 1 registered on backend A and a user 2 registered on backend B must be able to interact with each other as if they belonged to the same backend.
