      ...
      $ forM_
        [ (h, u, e)
          | h <- [False, True], -- are users scim provisioned or via team management invitations?
            ...
            (h, u) /= (True, False) -- scim doesn't not work with more than one idp (there are tickets for that somewhere)
        ]
      $ \(haveScim, updateNotReplace, externalIdIsEmail) -> do
        focus $ it ("creates new idp, setting old_issuer; sets replaced_by in old idp; scim user search still works " <> show (haveScim, updateNotReplace, externalIdIsEmail)) $ do
          env <- ask
          ...
          liftIO $ do
                             ...
                             & if updateNotReplace
                               then
                                 (idpMetadata . edIssuer .~ (idp2' ^. idpMetadata . edIssuer))
                                   . (idpExtraInfo . wiOldIssuers .~ [idp1 ^. idpMetadata . edIssuer])
                               else idpExtraInfo . wiReplacedBy .~ idp1' ^. idpExtraInfo . wiReplacedBy
                         )
            idp2'
              `shouldBe` ( idp2
                             & if updateNotReplace
            ...
            idp1' ^. idpExtraInfo . wiReplacedBy
