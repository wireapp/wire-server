...
httpErrorToSparError :: HttpError -> SparError
httpErrorToSparError = SAML.CustomError . SparSomeHttpError
...
sparToServerErrorWithLogging logger err = do
  ...
  liftIO $ logHttpError logger (Nothing :: Maybe Wai.Request) (renderSparError err)
  pure errServant
...
renderSparError :: SparError -> HttpError
renderSparError (SAML.CustomError SparNoSuchRequest) = StdError $ Wai.mkError status500 "server-error" "AuthRequest seems to have disappeared (could not find verdict format)."
...
renderSparError (SAML.CustomError (SparBadInitiateLoginQueryParams label)) = StdError $ Wai.mkError status400 label label
renderSparError (SAML.CustomError (SparUserRefInNoOrMultipleTeams msg)) = StdError $ Wai.mkError status403 "bad-team" ("Forbidden: multiple teams or no team for same UserRef " <> msg)
renderSparError (SAML.CustomError (SparBadUserName msg)) = StdError $ Wai.mkError status400 "bad-username" ("Bad UserName in SAML response, except len [1, 128]: " <> msg)
renderSparError (SAML.CustomError (SparCannotCreateUsersOnReplacedIdP replacingIdPId)) = StdError $ Wai.mkError status400 "cannont-provision-on-replaced-idp" ("This IdP has been replaced, users can only be auto-provisioned on the replacing IdP " <> replacingIdPId)
-- RFC-specific errors
...
renderSparError (SAML.CustomError (SparCassandraError msg)) = StdError $ Wai.mkError status500 "server-error" msg -- TODO: should we be more specific here and make it 'db-error'?
...
renderSparError SAML.BadSamlResponseFormFieldMissing = StdError $ Wai.mkError status400 "bad-response-saml" "Bad response: SAMLResponse form field missing from HTTP body"
renderSparError SAML.BadSamlResponseIssuerMissing = StdError $ Wai.mkError status400 "bad-response-saml" "Bad response: no Issuer in AuthnResponse"
renderSparError SAML.BadSamlResponseInconsistentIdPIssuerInfo = StdError $ Wai.mkError status403 "bad-response-saml" "Bad response: IdP Issuer in AuthnResponse does not match AuthnRequest"
renderSparError SAML.BadSamlResponseNoAssertions = StdError $ Wai.mkError status400 "bad-response-saml" "Bad response: no assertions in AuthnResponse"
renderSparError SAML.BadSamlResponseAssertionWithoutID = StdError $ Wai.mkError status400 "bad-response-saml" "Bad response: assertion without ID"
renderSparError (SAML.BadSamlResponseInvalidSignature msg) =
  StdError $
    Wai.mkError status400 "bad-response-signature" msg
renderSparError (SAML.CustomError (SparIdPNotFound "")) = StdError $ Wai.mkError status404 "not-found" "Could not find IdP."
renderSparError (SAML.CustomError (SparIdPNotFound msg)) = StdError $ Wai.mkError status404 "not-found" ("Could not find IdP: " <> msg)
renderSparError (SAML.CustomError (SparSPNotFound "")) = StdError $ Wai.mkError status404 "not-found" "Could not find SP."
renderSparError (SAML.CustomError (SparSPNotFound msg)) = StdError $ Wai.mkError status404 "not-found" ("Could not find SP: " <> msg)
renderSparError (SAML.CustomError SparSamlCredentialsNotFound) = StdError $ Wai.mkError status404 "not-found" "Could not find SAML credentials, and auto-provisioning is disabled."
renderSparError (SAML.CustomError SparMissingZUsr) = StdError $ Wai.mkError status400 "client-error" "[header] 'Z-User' required"
renderSparError (SAML.CustomError SparNotInTeam) = StdError $ Wai.mkError status403 "no-team-member" "Requesting user is not a team member or not a member of this team."
...
renderSparError (SAML.CustomError SparSSODisabled) = StdError $ Wai.mkError status403 "sso-disabled" "Please ask customer support to enable this feature for your team."
renderSparError SAML.UnknownError = StdError $ Wai.mkError status500 "server-error" "Unknown server error."
renderSparError (SAML.BadServerConfig msg) = StdError $ Wai.mkError status500 "server-error" ("Error in server config: " <> msg)
renderSparError (SAML.InvalidCert msg) = StdError $ Wai.mkError status500 "invalid-certificate" ("Error in idp certificate: " <> msg)
-- Errors related to IdP creation
renderSparError (SAML.CustomError (SparNewIdPBadMetadata msg)) = StdError $ Wai.mkError status400 "invalid-metadata" msg
renderSparError (SAML.CustomError SparNewIdPPubkeyMismatch) = StdError $ Wai.mkError status400 "key-mismatch" "public keys in body, metadata do not match"
renderSparError (SAML.CustomError (SparNewIdPAlreadyInUse msg)) = StdError $ Wai.mkError status400 "idp-already-in-use" msg
renderSparError (SAML.CustomError (SparNewIdPWantHttps msg)) = StdError $ Wai.mkError status400 "idp-must-be-https" ("an idp request uri must be https, not http or other: " <> msg)
renderSparError (SAML.CustomError SparIdPHasBoundUsers) = StdError $ Wai.mkError status412 "idp-has-bound-users" "an idp can only be deleted if it is empty"
renderSparError (SAML.CustomError SparIdPIssuerInUse) = StdError $ Wai.mkError status400 "idp-issuer-in-use" "The issuer of your IdP is already in use.  Remove the entry in the team that uses it, or construct a new IdP issuer."
renderSparError (SAML.CustomError SparIdPCannotDeleteOwnIdp) = StdError $ Wai.mkError status409 "cannot-delete-own-idp" "You cannot delete the IdP used to login with your own account."
renderSparError (SAML.CustomError (IdpDbError InsertIdPConfigCannotMixApiVersions)) = StdError $ Wai.mkError status409 "cannot-mix-idp-api-verions" "You cannot have two IdPs with the same issuerwhere one of them is using API V1 and one API V2."
renderSparError (SAML.CustomError (IdpDbError AttemptToGetV1IssuerViaV2API)) = StdError $ Wai.mkError status409 "cannot-mix-idp-api-verions" "You cannot retrieve an API V1 IdP via API V2."
renderSparError (SAML.CustomError (IdpDbError AttemptToGetV2IssuerViaV1API)) = StdError $ Wai.mkError status409 "cannot-mix-idp-api-verions" "You cannot retrieve an API V2 IdP via API V1."
...
renderSparError (SAML.CustomError (SparMultiIngressIdPConfiguration msg)) = StdError $ Wai.mkError status400 "multi-ingress-config-error" ("Multi-ingress SSO requires email-based NameIDs: " <> msg)
...
renderSparError (SAML.CustomError SparProvisioningTokenLimitReached) = StdError $ Wai.mkError status403 "token-limit-reached" "The limit of provisioning tokens per team has been reached"
-- SCIM errors
renderSparError (SAML.CustomError (SparScimError err)) = case Scim.scimToWaiError err of
  (e, hs) -> RichError e () hs
renderSparError (SAML.CustomError (SparInternalError err)) = StdError $ Wai.mkError status500 "server-error" ("Internal error: " <> err)
