...
assertSSOEnabled tid = do
  ...
  (TeamFeatureStatus status _) :: (TeamFeatureStatus 'TeamFeatureSSO) <- parseResponse resp
...
isEmailValidationEnabledTeam tid = do
  ...
  pure (statusCode resp == 200 && responseJsonMaybe @(TeamFeatureStatus 'TeamFeatureValidateSAMLEmails) resp == Just (TeamFeatureStatus TeamFeatureEnabled ()))
