...
type ScimTokenRow = (ScimToken, TeamId, ScimTokenId, Maybe SAML.IdPId, Text)
...
insertScimToken token ScimTokenInfo{..} = retry x5 $ batch $ do
    setType BatchLogged
    setConsistency Quorum
    addPrepQuery insByToken (token, stiTeam, stiId, stiIdP, stiDescr)
    addPrepQuery insByTeam  (token, stiTeam, stiId, stiIdP, stiDescr)
  where
    insByToken, insByTeam :: PrepQuery W ScimTokenRow ()
    insByToken = "INSERT INTO team_provisioning_by_token \
                 \(token_, team, id, idp, descr) VALUES (?, ?, ?, ?, ?)"
...
lookupScimToken token = do
    ...
    pure $ fmap (\(_, stiTeam, stiId, stiIdP, stiDescr) -> ScimTokenInfo{..}) mbRow
  where
    sel :: PrepQuery R (Identity ScimToken) ScimTokenRow
    sel = "SELECT token_, team, id, idp, descr \
...
getScimTokens team = do
    ...
    pure $ map (\(_, stiTeam, stiId, stiIdP, stiDescr) -> ScimTokenInfo{..}) rows
  where
