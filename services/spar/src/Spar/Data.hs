...
type ScimTokenRow = (TeamId, ScimToken, Maybe SAML.IdPId)
...
insertScimToken team token idp =
  retry x5 . write ins $ params Quorum (team, token, idp)
  where
    ins :: PrepQuery W ScimTokenRow ()
    ins = "INSERT INTO team_provisioning (team, token, idp) VALUES (?, ?, ?)"

-- | Check whether a token exists and if yes, what IdP is associated with it.
lookupScimToken
  :: (HasCallStack, MonadClient m)
  => TeamId -> ScimToken -> m (Maybe ScimTokenRow)
lookupScimToken team token =
  retry x1 . query1 sel $ params Quorum (team, token)
  where
    sel :: PrepQuery R (TeamId, ScimToken) ScimTokenRow
    sel = "SELECT team, token, idp FROM team_provisioning \
          \WHERE team = ? AND token = ?"
