...
type ScimTokenRow = (TeamId, ScimToken, Maybe SAML.IdPId, Text)
...
lookupScimToken token = do
    ...
    pure $ fmap (\(stiTeam, stiToken, stiIdP, stiDescr) -> ScimTokenInfo{..}) mbRow
  where
...
getScimTokens team = do
    ...
    pure $ map (\(stiTeam, stiToken, stiIdP, stiDescr) -> ScimTokenInfo{..}) rows
  where
