...
getIdPConfig ::
  ...
  IdPId ->
  Sem r IdP
getIdPConfig = (>>= maybe (throwSparSem (SparIdPNotFound mempty)) pure) . IdConfigStore.getConfig

storeIdPConfig :: Member IdConfigStore r => IdP -> Sem r ()
storeIdPConfig idp = IdConfigStore.storeConfig idp

getIdPConfigByIssuerOptionalSPId :: Members '[IdConfigStore, Error SparError] r => Issuer -> Maybe TeamId -> Sem r IdP
...
getIdPIdByIssuerAllowOld ::
  (HasCallStack) =>
  Member IdConfigStore r =>
  SAML.Issuer ->
  Maybe TeamId ->
  Sem r (GetIdPResult SAML.IdPId)
getIdPIdByIssuerAllowOld issuer mbteam = do
  mbv2 <- maybe (pure Nothing) (IdConfigStore.getIdByIssuerWithTeam issuer) mbteam
  mbv1v2 <- maybe (IdConfigStore.getIdByIssuerWithoutTeam issuer) (pure . GetIdPFound) mbv2
  case (mbv1v2, mbteam) of
    (GetIdPFound idpid, Just team) -> do
      IdConfigStore.getConfig idpid >>= \case
        Nothing -> do
...
getIdPConfigByIssuer ::
  (HasCallStack, Member IdConfigStore r) =>
  SAML.Issuer ->
  TeamId ->
...
mapGetIdPResult :: (HasCallStack, Member IdConfigStore r) => GetIdPResult SAML.IdPId -> Sem r (GetIdPResult IdP)
mapGetIdPResult (GetIdPFound i) = IdConfigStore.getConfig i <&> maybe (GetIdPDanglingId i) GetIdPFound
...
deleteTeam ::
  (HasCallStack, Members '[ScimTokenStore, SAMLUserStore, IdConfigStore] r) =>
  TeamId ->
  Sem r ()
deleteTeam team = do
  ...
  idps <- IdConfigStore.getConfigsByTeam team
  for_ idps $ \idp -> do
    let issuer = idp ^. SAML.idpMetadata . SAML.edIssuer
    SAMLUserStore.deleteByIssuer issuer
    IdConfigStore.deleteConfig idp
