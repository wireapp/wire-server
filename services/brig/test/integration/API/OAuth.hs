...
createTeamConv svc mkHeader token tid name = do
  ...
  let conv = Conv.NewConv [] [] (checked name) (Set.fromList [CodeAccess]) Nothing (Just tinfo) Nothing Nothing Role.roleNameWireAdmin BaseProtocolProteusTag GroupConversation False
  post $
    svc
      . path "conversations"
      . mkHeader token
