...
testInvitationEmail brig = do
  ...
  invitee <- randomEmail
  let invite = InvitationRequest Nothing Nothing Nothing invitee Nothing
...
testInvitationEmailLookup brig = do
  ...
  let invite = InvitationRequest Nothing Nothing Nothing email Nothing
  ...
  let invite2 = InvitationRequest Nothing Nothing Nothing email Nothing
...
testInvitationTooManyPending brig (TeamSizeLimit limit) = do
  ...
  let invite email = InvitationRequest Nothing Nothing Nothing email Nothing
...
testInvitationRoles brig galley = do
  ...
  alice :: UserId <- do
    ...
    let invite = InvitationRequest Nothing (Just Team.RoleAdmin) Nothing aliceEmail Nothing
    inv :: Invitation <- responseJsonError =<< postInvitation brig tid owner invite
  ...
  do
    ...
    let invite = InvitationRequest Nothing (Just Team.RoleExternalPartner) Nothing bobEmail Nothing
    inv :: Invitation <-
      responseJsonError
  ...
  do
    ...
    let invite = InvitationRequest Nothing (Just Team.RoleOwner) Nothing charlyEmail Nothing
    postInvitation brig tid alice invite !!! do
      const 403 === statusCode
...
testInvitationEmailAndPhoneAccepted brig galley = do
  ...
  let stdInvite = InvitationRequest Nothing Nothing Nothing inviteeEmail Nothing
...
testInvitationPaging brig = do
  ...
  (uid, tid) <- createUserWithTeam brig
  let total = 5
      invite email = InvitationRequest Nothing Nothing Nothing email Nothing
...
testSuspendTeam brig = do
  ...
  let invite2 = InvitationRequest Nothing Nothing Nothing inviteeEmail2 Nothing
