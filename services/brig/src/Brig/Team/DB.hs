...
insertInvitation t email (toUTCTimeMillis -> now) minviter timeout = do
    iid  <- liftIO mkInvitationId
    code <- liftIO mkInvitationCode
    let inv = Invitation t iid email now minviter
    retry x5 $ batch $ do
        setType BatchLogged
        setConsistency Quorum
        addPrepQuery cqlInvitation (t, iid, code, email, now, minviter, round timeout)
  ...
  where
    ...
    cqlInvitation :: PrepQuery W (TeamId, InvitationId, InvitationCode, Email, UTCTimeMillis, Maybe UserId, Int32) ()
    cqlInvitation = "INSERT INTO team_invitation (team, id, code, email, created_at, created_by) VALUES (?, ?, ?, ?, ?, ?) USING TTL ?"
