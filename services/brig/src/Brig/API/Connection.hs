...
createConnectionToLocalUser self crUser ConnectionRequest {crName, crMessage} conn = do
  ...
  where
    insert s2o o2s = lift $ do
      ...
      e2o <- ConnectionUpdated o2s' (ucStatus <$> o2s) <$> Data.lookupName self Nothing
      e2s <- pure $ ConnectionUpdated s2o' (ucStatus <$> s2o) Nothing Nothing
    ...
    accept s2o o2s = do
      ...
      e2o <- lift $ ConnectionUpdated o2s' (Just $ ucStatus o2s) <$> Data.lookupName self Nothing
      e2s <- pure $ ConnectionUpdated s2o' (Just $ ucStatus s2o) Nothing Nothing
...
updateConnection self other newStatus conn = do
  ...
  lift . for_ s2o' $ \c ->
    let e2s = ConnectionUpdated c (Just $ ucStatus s2o) Nothing (error "TODO")
  ...
  where
    accept s2o o2s = do
      ...
      when (ucStatus o2s `elem` [Sent, Pending]) . lift $ do
        ...
        e2o <- ConnectionUpdated o2s' (Just $ ucStatus o2s) <$> Data.lookupName self Nothing
