...
createUserSpar new = do
  ...
  -- Add to team
  userTeam <-
    let email = Nothing -- no *validated* email can exist yet in the sso case.
     in withExceptT CreateUserSparRegistrationError $ addUserToTeamSSO account tid (UAuthIdentity ident Nothing) (newUserSparRole new)
  ...
  where
    ...
    addUserToTeamSSO :: UserAccount -> TeamId -> UserIdentity tf -> Role -> ExceptT RegisterError (AppT r) CreateUserTeam
...
createUser new = do
          ...
          let existingUser = accountUser existingAccount
           in ( new
                  { newUserManagedBy = Just (userManagedBy existingUser),
                    newUserIdentity = newIdentity email phone (newUserUAuthId new)
                  },
  ...
  createUserTeam <- do
    ...
    joinedTeamSSO <- case (newUserIdentity new', tid) of
      (Just ident@(UAuthIdentity (UAuthId (Just _) _ _ tid'') _), Just tid') ->
        assert (tid'' == tid') $ Just <$> addUserToTeamSSO account tid' ident
      _ -> pure Nothing
  ...
  where
    ...
    addUserToTeamSSO :: UserAccount -> TeamId -> UserIdentity tf -> ExceptT RegisterError (AppT r) CreateUserTeam
...
-- 'UAuthid' can still do this if they also have email and password.  Otherwise,
...
deleteSelfUser uid pwd = do
  ...
  where
    ...
    getEmailOrPhone :: UserIdentity tf -> Maybe (Either Email Phone)
