...
createUser new@NewUser{..} = do
    ...
    Log.debug $ field "user" (toByteString uid) . msg (val "User.createUser")
...
onActivated (AccountActivated account) = do
    let uid = userId (accountUser account)
    Log.debug $ field "user" (toByteString uid) . msg (val "User.onActivated")
...
beginPasswordReset target = do
    ...
    Log.debug $ field "user" (toByteString user) . msg (val "User.beginPasswordReset")
...
completePasswordReset ident code pw = do
    ...
    case muid of
        ...
        Just uid -> do
            Log.debug $ field "user" (toByteString uid) . msg (val "User.completePasswordReset")
