...
newtype IndexIO a = IndexIO (ReaderT IndexEnv IO a)
    deriving (Functor, Applicative, Monad, MonadIO, MonadReader IndexEnv,
...
checkIndex u = liftIndexIO $ fmap SearchableStatus $ do
    ...
    case statusCode (responseStatus r) of
        404 -> pure False
        _   -> ES.parseEsResponse r
           >>= either (throwM . IndexLookupError)
...
queryIndex :: (MonadIndexIO m, FromJSON r)
    => IndexQuery r
    -> Range 1 100 Int32
    -> m (SearchResult r)
queryIndex (IndexQuery q f) (fromRange -> s) = liftIndexIO $ do
    idx <- asks idxName
    r <- ES.searchByType idx mappingName
...
defaultUserQuery u (normalized -> term') = mkUserQuery u $ ES.QueryBoolQuery boolQuery
...
mkUserQuery (review _TextId -> self) q = IndexQuery q $
  ...
  where
    termQ f v = ES.TermQuery ES.Term
        { ES.termField = f
        , ES.termValue = v
        } Nothing
...
updateIndex (IndexUpdateUser iu) = liftIndexIO $ do
    ...
    info $ field "user" (Bytes.toByteString (view iuUserId iu))
  ...
  where
    versioning = ES.defaultIndexDocumentSettings
...
updateIndex (IndexUpdateUsers ius) = liftIndexIO $ do
    ...
    info $ field "num_users" (length ius)
  ...
  where
    ...
    bulkMeta docId v = fromEncoding . pairs . pair "index" . pairs $
           "_id"           .= docId
        <> "_version"      .= v
...
resetIndex s = liftIndexIO $ do
    ...
    if gone then
        ...
        throwM (IndexError "Index deletion failed.")
...
reindexAll = do
  ...
  where
    loop idx page = do
        info $ field "size" (length (C.result page))
...
lookupForIndex u = fmap reindexRowToIndexUser <$>
...
scanForIndex num = fmap reindexRowToIndexUser <$>
...
reindexRowToIndexUser
    (u, name, t0, status, t1, handle, t2, searchable, t3, colour, t4, activated, t5, service, t6)
    = let iu = mkIndexUser u (version [Just t0, t1, t2, t3, Just t4, Just t5, t6])
       in if shouldIndex then
