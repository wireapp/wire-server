...
addBot zuid zcon cid add = do
  ...
  let usr = User (botUserId bid) (Qualified (botUserId bid) domain) Nothing name pict assets colour False locale (Just sref) Nothing Nothing Nothing ManagedByWire (error "TODO: what now?")
  let newClt =
