...
module Brig.Effects.FireAndForget
  ( FireAndForget,
    fireAndForget,
    spawnMany,
    spawnAndRemember,
...
data FireAndForget m a where
  ...
  -- TODO(sandy): This is a bad place for this effect, or the name is wrong.
  -- And the name is wrong.
  SpawnAndRemember :: [m a] -> FireAndForget m [a]
...
interpretFireAndForget = interpretFinal @IO $ \case
  ...
  SpawnAndRemember actions -> do
    actions' <- traverse runS actions
    ins <- getInspectorS
    -- I picked this number by fair dice roll, feel free to change it :P
    liftS $ fmap catMaybes $ pooledMapConcurrentlyN 8 (fmap $ inspect ins) actions'
