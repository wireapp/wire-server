...
instance MonadPushAll Gundeck where  -- TODO: double-check these if they are the right boundary, or
                                     -- whether e.g. Stream.add should also be in MonadPushAll, or
                                     -- some other class.

  mpaMkNotificationId = mkNotificationId
  mpaListAllPresences = Presence.listAll
  mpaBulkPush         = Web.bulkPush  -- TODO: there may be more stuff here to mock and test, but
                                      -- perhaps we can do it locally in that module, independently
                                      -- of the tests here.  or would it save work to do it
                                      -- together?

                                      -- TODO: there may be some pure functions there that need no
                                      -- change of production code for having better unit tests!
