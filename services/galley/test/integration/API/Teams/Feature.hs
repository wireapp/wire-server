      ...
      test s "MlsE2EId feature config" $
        testNonTrivialConfigNoTTL
          ( withStatus
              ...
              LockStatusUnlocked
              (MlsE2EIdConfig (fromIntegral @Int (60 * 60 * 24)))
              FeatureTTLUnlimited
          ),
      testGroup
        "Patch"
        [ -- Note: `SSOConfig` and `LegalHoldConfig` may not be able to be reset
          ...
          test s (unpack $ featureNameBS @MlsE2EIdConfig) $ testPatchWithArbitrary AssertLockStatusChange FeatureStatusDisabled (MlsE2EIdConfig (fromIntegral @Int (60 * 60 * 24)))
        ],
      testGroup
    ...
    expected confCalling lockStateSelfDeleting =
      AllFeatureConfigs
        { afcLegalholdStatus = withStatus FeatureStatusDisabled LockStatusUnlocked LegalholdConfig FeatureTTLUnlimited,
          ...
          afcMlsE2EId = withStatus FeatureStatusDisabled LockStatusUnlocked (MlsE2EIdConfig (fromIntegral @Int (60 * 60 * 24))) FeatureTTLUnlimited
        }
