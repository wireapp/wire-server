  ...
  WS.bracketR2 c owner extern $ \(wsOwner, wsExtern) -> do
    lConvId <-
      Util.createMLSTeamConv
        ...
        Nothing
        Nothing
    Right (conv :: OwnConversation GroupConvType) <- responseJsonError <$> getConvQualified owner (tUntagged lConvId)
    liftIO $ do
    ...
    WS.assertNoEventExcept (2 # Second) [wsExtern] $ wsIsEventOfType "user.activate"
