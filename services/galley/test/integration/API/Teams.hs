...
testEnableSSOPerTeam = do
  ...
  let check :: (HasCallStack) => String -> Public.FeatureStatus -> TestM ()
      check msg enabledness = do
        status :: Public.Feature Public.SSOConfig <- responseJsonUnsafe <$> (getSSOEnabledInternal tid <!! testResponse 200 Nothing)
          ...
          responseJsonUnsafe
            <$> put
              ( g
                  ...
                  . json (Public.Feature Public.FeatureStatusDisabled Public.SSOConfig Public.FeatureTTLUnlimited)
              )
        liftIO $ do
...
testEnableTeamSearchVisibilityPerTeam = do
  ...
  let check :: String -> Public.FeatureStatus -> TestM ()
      check msg enabledness = do
        status :: Public.Feature Public.SearchVisibilityAvailableConfig <- responseJsonUnsafe <$> (Util.getTeamFeatureInternal @Public.SearchVisibilityAvailableConfig tid <!! testResponse 200 Nothing)
        let statusValue = Public.wssStatus status
...
putSSOEnabledInternal tid statusValue =
  void $ Util.putTeamFeatureInternal @Public.SSOConfig expect2xx tid (Public.Feature statusValue Public.SSOConfig Public.FeatureTTLUnlimited)
