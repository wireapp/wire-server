  ...
  when (null conv.metadata.cnvmParent) $
    mapErrorS @OperationDenied @('ActionDenied 'AddConversationMember) $
      case conv.metadata.cnvmTeam of
        Nothing -> void $ permissionCheck JoinRegularConversations (Nothing @TeamMember)
        Just tid -> forM_ users $ permissionCheck JoinRegularConversations <=< E.getTeamMember tid . qUnqualified
