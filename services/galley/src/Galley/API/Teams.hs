...
import Wire.API.Event.Conversation (evtType, isPydioEvent)
...
uncheckedDeleteTeam lusr zcon tid = do
  ...
  where
    ...
    createConvDeleteEvents now teamMembs c (pp, ee) = do
      ...
      let p = newPushLocal (tUnqualified lusr) (toJSONObject e) (map localMemberToRecipient mm) (isPydioEvent $ evtType e)
      let ee' = map (,e) bots
