...
CHARTS_INTEGRATION    := wire-server databases-ephemeral rabbitmq fake-aws ingress-nginx-controller nginx-ingress-services wire-ingress fluent-bit opensearch-dashboards k8ssandra-test-cluster wire-server-enterprise
...
fake-aws fake-aws-s3 fake-aws-sqs aws-ingress fluent-bit opensearch-dashboards backoffice		\
calling-test demo-smtp opensearch-curator opensearch-external					\
opensearch-ephemeral minio-external cassandra-external							\
