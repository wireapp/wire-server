...
For OTP verification using Twilio, you need to specify the sid and token from the Twilio account in the "wire-server/services/brig/test/resources/twilio-credentials.yaml ". And specify your Twilio number in smsSender in file "wire-server/deploy/services-demo/conf/brig.demo.yaml" at `emailSMS.general.smsSender`. 
