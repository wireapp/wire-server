...
testInvitationUrl :: HasCallStack => App ()
testInvitationUrl = do
  -- this is really hard to test: we need to change the config in a dynamic backend while it's running.
  (_owner, tid, []) <- createTeam domain 1
  startDynamicBackends [def {galleyCfg = \cfg -> cfg & settings . exposeInvitationURLsTeamAllowlist .~ []}] $ \[domain] -> do
    setTeamFeatureStatus domain tid "exposeInvitationURLsToTeamAdmin" "enabled" >>= assertSuccess
    bindResponse (getTeamFeature domain tid "exposeInvitationURLsToTeamAdmin") $ \resp -> do
      error $ show resp
