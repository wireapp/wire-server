...
testAccessUpdateGuestRemoved = do
  ...
  let assertNotification ::
        (HasCallStack, MakesValue user, MakesValue kickedUser) =>
        user ->
        String ->
        kickedUser ->
        App ()
      assertNotification user client kickedUser =
        void $
          awaitNotification
            user
            client
            ...
            ( allPreds
                [ isConvLeaveNotif,
                  ...
                  isNotifFromUser alice
                ]
            )

  mapM_ (assertNotification alice aliceClient) [charlie, dee]
  mapM_ (assertNotification bob bobClient) [charlie, dee]
  mapM_ (assertNotification charlie charlieClient) [charlie, dee]
  mapM_ (assertNotification dee deeClient) [charlie, dee]
