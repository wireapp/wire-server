...
getCookieWithSaml :: (HasCallStack) => String -> String -> (String, (SAML.IdPMetadata, SAML.SignPrivCreds)) -> App (Maybe String)
getCookieWithSaml tid email idp =
  fst <$> getCookieWithSamlLogin Nothing OwnDomain True tid (fromRight (error "could not create name id") $ SAML.emailNameID (cs email)) idp
...
loginWithSamlWithZHost mbZHost domain expectSuccess tid nameId idp = do
  (mCookie, authnReqResp) <- getCookieWithSamlLogin mbZHost domain expectSuccess tid nameId idp
...
getCookieWithSamlLogin ::
  ...
  (String, (SAML.IdPMetadata, SAML.SignPrivCreds)) ->
  App (Maybe String, SAML.SignedAuthnResponse)
getCookieWithSamlLogin mbZHost domain expectSuccess tid nameId (iid, (meta, privcreds)) = do
