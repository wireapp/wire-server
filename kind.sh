...
for i in $(seq 1 2); do
  helm upgrade --install --namespace "fed${i}" fake-aws wire-develop/fake-aws --devel --wait --values ./values.yaml
done
for i in $(seq 1 2); do
  helm upgrade --install --namespace "fed${i}" databases-ephemeral wire-develop/databases-ephemeral --devel --wait --values ./values.yaml
done
for i in $(seq 1 2); do
  helm upgrade \
    --install \
