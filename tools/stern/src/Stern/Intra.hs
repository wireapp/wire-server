...
catchRpcErrors action = ExceptT $ catch (Right <$> action) catchRPCException
  where
    ...
    catchRPCException rpcE = do
      ...
      pure . Left $ mkError status500 "io-error" (pack $ displayExceptionNoBacktrace rpcE)
...
getUserConversations :: UserId -> Int -> Handler [OwnConversation GroupConvType]
getUserConversations uid maxConvs = do
  info $ msg "Getting user conversations"
  fetchAll [] Nothing maxConvs
  where
    fetchAll :: [OwnConversation GroupConvType] -> Maybe ConvId -> Int -> Handler [OwnConversation GroupConvType]
    ...
    fetchBatch :: Maybe ConvId -> Int -> Handler (ConversationList (OwnConversation GroupConvType))
...
domRegGet :: Domain -> SC.ClientM (DomainRegistrationResponse V10)
( domRegLock
    :<|> domRegUnlock
    :<|> domRegPreAuthorize
    :<|> domRegUnauthorize
