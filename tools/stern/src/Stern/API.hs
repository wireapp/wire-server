...
import Network.Wai.Predicate hiding (Error, setStatus, reason)
...
sitemap = do
    ...
    get "/users" (continue usersByPhone)
        phoneParam
    ...
    document "POST" "revokeIdentity" $ do
        ...
        Doc.notes "Forcefully revokes a verified user identity. \
    ...
    document "PUT" "changeEmail" $ do
        ...
        Doc.notes "The new e-mail address must be verified \
    ...
    document "PUT" "changePhone" $ do
        ...
        Doc.notes "The new phone number must be verified \
                    \before the change takes effect."
        Doc.parameter Doc.Path "uid" Doc.bytes' $
            Doc.description "User ID"
    ...
    case usrs of
        ((accountUser -> u):_) ->
          if checkUUID u then do
              ...
              throwE $ Error status400 "match-error" "email or phone did not match UserId"
...
getConsentLog e = do
    ...
    when (isJust acc) $
        throwE $ Error status403 "user-exists" "Trying to access consent log of existing user!"
    consentLog <- Intra.getEmailConsentLog e
    marketo    <- Intra.getMarketoResult e
    return . json $ object [ "consent_log" .= consentLog
                           , "marketo"     .= marketo
                           ]
