...
tests s =
  testGroup
    ...
    [ test s "GET /i/status" testGetStatus,
      ...
      test s "/teams/:tid/features/channels" $ testFeatureConfig @ChannelsConfig,
      test s "PUT /teams/:tid/features/channels{,'?lockOrUnlock'}" $ testFeatureStatusWithLock @ChannelsConfig
