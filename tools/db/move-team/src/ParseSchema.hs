...
read{{keySpaceCaml}}{{tableNameCaml}} :: {{lookupKeyType}} -> Client (Maybe Row{{keySpaceCaml}}{{tableNameCaml}})
read{{keySpaceCaml}}{{tableNameCaml}} {{lookupKeyVar}} =
  retry x1 (query1 select{{keySpaceCaml}}{{tableNameCaml}} (params Quorum (pure {{lookupKeyVar}})))
