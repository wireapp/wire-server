    ...
    arbitrary = do
      ...
      where
        stripQuotes t = case T.strip t of
            t' | "\"" `T.isPrefixOf` t' -> either error id . unquote $ t'
               | otherwise              -> t'
...
instance Arbitrary (ParseInput (SvLogRecord TinyLogRecord)) where
    arbitrary = do
        ...
        ParseInput (sv  , _     ) <- (arbitrary :: Gen (ParseInput (SvLogRecord Text)))
...
mkSvInput df rec msg = mconcat
    [ maybe "" ((<> " ") . dateFormat df) (svTime rec)
    , if null (svTags rec) then B.empty else mconcat
        [ "|"
        , BC.intercalate "," $
...
instance Arbitrary (ParseInput (NginzLogRecord)) where
    arbitrary = do
        ...
        let rec = CommonLogRecord
                { cTime    = date
                , cFields  = mapMaybe (\ (k,v) -> (,) k <$> fromField v)
                           $ ("remote_addr", raddr)
                           : ("remote_user", ruser)
                           : fields
                , cRequest = req
      ...
      where
        ...
        genStringField =
                maybe CEmpty (\ s -> let s' = utf8 . unquoted $ s
                                      in if s' == "" || s' == "-"
                                           then CEmpty
        ...
        unField (CField (Number n)) = encodeUtf8 . toStrict . toLazyText $
            either (const $ scientificBuilder n)
                   T.decimal
...
mkSockInput df rec msg = mconcat
    [ dateFormat df . sockTime $ rec
    , " "
    , maybe BC.empty ((<> ": ") . encodeUtf8 . host) $ sockOrigin rec
    , if null (sockTags rec) then BC.empty else mconcat
        [ "|"
        , BC.intercalate ","
...
instance Arbitrary (ParseInput TaggedNetstring) where
    arbitrary = do
      ...
      where
        ...
        netstr' (Just k , v) = B.intercalate (netstr "=")
                             . map (netstr . taggedValue)
                             $ [k,v]


netstr :: ByteString -> ByteString
netstr bs = BL.toStrict
          . B.toLazyByteString
...
instance Arbitrary (ParseInput TinylogNetstr) where
    arbitrary = do
        ...
        let rec = TinylogNetstr TinyLogRecord
                { tDate    = fmap (decodeUtf8 . df $) date
                , tLevel   = level
                , tFields  = mapMaybe (\ (k,v) -> case k of
                                 Just k' -> Just (toText (taggedValue k'), toText (taggedValue v))
                                 _       -> Nothing)
                           $ fields
                , tMessage = T.intercalate ", "
...
instance SecondsPrecision a => SecondsPrecision (SockLogRecord a) where
    secs rec = rec { sockTime    = secs' round (sockTime rec)
