...
Open `my-wire-server/values.yaml` and replace `wire.example` and other domains and subdomains with domains of your choosing. Look for the `# change this` comments. You can try using `sed -i 's/wire.example/<your-domain>/g' values.yaml`.
