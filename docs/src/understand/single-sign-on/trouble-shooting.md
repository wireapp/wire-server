...
>   * [Can I use SAML without SCIM?](#can-i-use-saml-without-scim)
>   * [Can I use the same SSO login code for multiple teams?](#can-i-use-the-same-sso-login-code-for-multiple-teams)
>   * [Can an existing user without IdP (or with a different IdP) be bound to a new IdP?](#can-an-existing-user-without-idp-or-with-a-different-idp-be-bound-to-a-new-idp)
>   * [Can the SSO feature be disabled for a team?](#can-the-sso-feature-be-disabled-for-a-team)
>   * [Do I need to change any firewall settings in order to use SAML?](#do-i-need-to-change-any-firewall-settings-in-order-to-use-saml)
>   * [Why does the team owner have to keep using password?](#why-does-the-team-owner-have-to-keep-using-password)
>   * [What should the SAML response look like?](#what-should-the-saml-response-look-like)
>   * [Why does the auth response not contain a reference to an auth request?  (Also: can i use IdP-initiated login?)](#why-does-the-auth-response-not-contain-a-reference-to-an-auth-request-also-can-i-use-idp-initiated-login)
>   * [How are SAML2 assertion details used in wire?](#how-are-saml2-assertion-details-used-in-wire)
>   * [How should I map user data to SCIM attributes when provisioning users via SCIM?](#how-should-i-map-user-data-to-scim-attributes-when-provisioning-users-via-scim)
...
- Have you followed the following instructions?
  - [FAQ](#trouble-shooting-faq) (This document)
  - [Howtos](./README.md) for supported vendors
...
- New query param `api_version=<v1|v2>` for `POST /identity-providers`.  The version is stored in `spar.idp` together
  with the rest of the IdP setup, and is used by `GET /sso/initiate-login` (see below).
